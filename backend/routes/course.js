...
router.get('/', function(req, res, next) {
    MongoClient.connect(url, function(err, db) {
        if (err) {
          ...
          dbo.collection("courses").find({_id:id},{ projection: {_id:1,name:1,tutor:1,subject:1,price:1,rating:1,link:1,video_buffer:1,student:1,photo_buffer:1,video_buffer:1,video_name:1,video_size:1,video_type:1} }).toArray(function(err, result) {
            if (err) {
              res.json({result:false , error:err})
            }
            db.close();
